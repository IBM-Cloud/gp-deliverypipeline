...
# rjm: move to untilities module perhaps 
...
MY_SERVICE='IBM Globalization'
MY_SERVICE_PLAN="Experimental"
...
def parseArgs ():
    parsedArgs = {}
    parsedArgs['all'] = False
    parsedArgs['check_space'] = False
    parsedArgs['submit_job'] = False
    parsedArgs['download_files'] = False
    for arg in sys.argv:
        if arg == "--all":
            parsedArgs['all'] = True 
        if arg == "--check_space":
            parsedArgs['check_space'] = True
        if arg == "--submit_job":
            parsedArgs['submit_job'] = True
        if arg == "--download_files":
            parsedArgs['download_files'] = True
    return parsedArgs
...
try:
    ...
    print "Getting credentials for globalization service"
    sys.stdout.flush()
    userid, password = getCredentialsFromBoundApp(service=MY_SERVICE)
    sys.stdout.flush()

    # allow testing without full submission
    if parsedArgs['check_space']:
        print "check_space set, login complete, exiting"
        sys.exit(0)
